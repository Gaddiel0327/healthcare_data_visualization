...
//     d3.csv("./data/Healthcare_CleanSet.csv").then(medicalCondition=> {
//         let med = medicalCondition.filter(d => d['Medical Condition'] === id);
...
/// Declare data variable outside of the d3.csv call to make it globally accessible
let data;

// Function to create a bar chart
function createBarChart(data) {
    let medicalConditions = data.map(d => d['Medical Condition']);
    let avgBillingAmounts = data.map(d => parseFloat(d['Billing Amount']));
...
function createPieChart(data) {
    let labels = data.map(d => d['Admission Type']);
    let values = data.map(d => parseFloat(d['Billing Amount']));
...
function createBubbleChart(data) {
    ...
    var trace1 = {
        ...
        marker: {
            ...
            colorbar: {
                title: 'Medical Condition',
                tickvals: uniqueMedicalConditions.map((_, index) => index),
                ticktext: uniqueMedicalConditions
            }
        }
    };
